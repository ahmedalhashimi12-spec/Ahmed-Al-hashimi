...
export default function SecondVideoScrubber({ scrollProgress }: SecondVideoScrubberProps) {
  ...
  // Request Animation Frame loop for smoothing video scrub from DRUM_START to DRUM_END
  useEffect(() => {
    let active = true;
    const tick = () => {
      if (!active) return;
      ...
      if (video && isLoaded) {
        ...
        const DRUM_END = 3.50;
        const drumProgress = clamp01((scrollProgress - DRUM_START) / (DRUM_END - DRUM_START));
  ...
  return (
    <div className="absolute inset-0 w-full h-full overflow-hidden bg-[#11010a]">
      <div
        ref={containerRef}
      ...
      {/* Loading Overlay */}
      {!isLoaded && (
        <div className="absolute inset-0 w-full h-full bg-[#11010af4] z-50 flex flex-col items-center justify-center gap-6">
          ...
          <span className="font-manrope font-bold text-[13px] text-pink-500 drop-shadow-[0_0_8px_rgba(234,31,99,0.4)]">
            جاري تهيئة العرض ثلاثي الأبعاد...
          </span>
        </div>
      )}
    </div>
